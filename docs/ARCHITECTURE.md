...
**Block-detail modal** (`detailItem`/`openTemplateDetail`/`openBlockDetail`/`closeDetail`/`selectExercise`, plus the `detailExercises` computed): clicking a palette item's ⓘ button or a placed block's 🔄 Swap button opens the app's first modal (no prior modal convention existed — see `.detail-modal-backdrop`/`.detail-modal` in `style.css`, patterned after the existing `.server-error` fixed-overlay + `.module-card` panel styles), listing every `EXERCISE_TEMPLATES` entry (`lib/exercises.js`, fetched via `GET /api/profiles/:id/training/exercises`) matching that block/template's category. (The placed-block ⓘ icon this originally shipped with was later replaced by a footer button bar — see "Block-card actions" below — the modal itself is unchanged.) `detailItem.blockId` is only set when opened from an already-*placed* block (not a palette template preview) — that's what gates the "Use this →" button per exercise row, which calls the normal `Storage.updateBlock` to overwrite that block's `title`/`details.exerciseId` with the chosen exercise. There's no separate per-block exercise table — the chosen exercise just becomes the block's title, same as picking a template does.

**Every `BLOCK_TEMPLATES` label has a matching `EXERCISE_TEMPLATES` entry, by design** — `resolvedExercise()`'s exact-name fallback (used by the 📊 Insights panel below, and by `workoutQueue.js`) only works when a template's `label` and an exercise's `name` coincide exactly. This was true for only 2 of the original 26 templates; the other 24 — including `heel_rehab`, which `autobuild.js`'s `dailyStretch()` places on *every day of every autobuilt week* — had no matching entry at all, so Insights showed "not linked to a specific exercise yet" on what's actually one of the most common blocks in the app. Fixed by adding one `exercises.js` entry per previously-unmatched template, name copied verbatim from `templates.js` (there's a one-off generator script recorded in this fix's commit message if a template's label ever needs to change — keeping the two in sync is a manual step, not automated, so re-run the same cross-check after editing either file: `BLOCK_TEMPLATES.map(t => t.label)` vs `EXERCISE_TEMPLATES.map(e => e.name)`, every label should appear in the name list).
...
**Palette categories are an accordion, collapsed by default — desktop only (≥1024px)** (`expandedCategories`, `toggleCategory()`) — `.palette-category-header` is now a `<button>`, not a plain label, showing an item count and toggling that category's items via `v-show` (same reasoning as the palette's own collapse above: keeps Sortable's DOM stable, no destroy/recreate). This was the first fix for the palette dumping all ~26 templates as one flat, single-column list on narrow screens (`.palette-item`'s `flex: 0 1 220px` only fits one per row under ~440px) — but even collapsed-by-category, opening one still grew the card taller inline, which on mobile still meant scrolling.

**Below 1024px, a category tile grid + modal replaces the accordion entirely** (`.palette-category-grid` for the tiles, `activeCategoryModal` for the drill-down modal — both in `weekBuilder.js`/`style.css`; `.palette-row`'s and `.palette-category-grid`'s complementary `@media` queries do the switching, not Tailwind classes, to avoid any load-order ambiguity between the static stylesheet and Tailwind's runtime-injected utility classes). Tapping a tile opens a modal listing just that category's templates (reusing `.detail-modal-backdrop`/`.detail-modal`) — the palette card's own height never changes no matter what's browsed, since the items render inside an overlay, not inline. `.palette-item` is reused as-is inside the modal's vertical list, with one override: its `flex: 0 1 220px` is a *width* basis in its normal home (`.palette-row`, a wrapping flex row) but became a *height* basis inside `.detail-modal-list` (a flex column) — `.detail-modal-list .palette-item { flex: none; width: 100%; }` fixes that. The items rendered in this modal are a second, independent `v-for` over the same `templatesByCategory` data — not the Sortable-tracked originals — so they're not drag sources; the ⓘ info and 📍 Place buttons (the latter closing this modal and opening the day/position placement modal described just below) are the only actions available here, which matches mobile's actual primary paths anyway.

**"📍 Place" is the mobile-first placement path** (`openPlacement()`/`placeWithOrder()`, `.placement-day-grid`/`.placement-slot-list`): every palette item now has a full-width Place button below its info row. Tapping it opens a small modal — pick a day (7 buttons with dates), then a position (an insert button before/after each of that day's existing blocks, interleaved with them) — and the block lands exactly there, all without ever scrolling down to a day column. This is additive, not a replacement: drag-and-drop and the existing tap-a-template-then-tap-a-day flow (below) are unchanged for anyone who prefers them.

**Run blocks (added 2026-07-30) let a user pick their own duration instead of a single fixed default** — `zone2_run`/`interval_run`/`tempo_run`/`long_run` (the "run" category) all carry a shared `durationOptions` array (`lib/templates.js`'s `RUN_DURATION_OPTIONS` — 15 through 180 minutes in 5-minute steps, 34 values) alongside their existing `defaultDurationMin`. This was **deliberately not implemented as 34 separate block templates** — one per duration — since that would balloon the "run" palette category from 4 tiles to 34+ (and the whole palette from ~26 items to ~56), the exact clutter problem the palette's own accordion/tile-grid/modal machinery (above) exists to avoid. Instead, any template carrying `durationOptions` gets a plain `<select>` dropdown wherever its duration is chosen:
- In the **"📍 Place" modal**, a "Choose a duration" `<select>` appears above the day picker whenever `placementItem.durationOptions` is set (`placementDurationMin`, reset on open/close) — this is the primary path for a duration-configurable template, since it already walks through day/position in discrete modal steps.
- In **tap-to-place**, selecting such a template reveals one dropdown (`tapDurationMin`) below the palette, applying to every subsequent "+ Add …" tap until deselected — a single shared control rather than repeating a `<select>` in all 7 day columns.
- **Drag-and-drop has no dialog step at all**, so a dragged run block always uses `defaultDurationMin`; anyone who wants a different duration for that placement uses Place or tap-to-place instead. This is a deliberate trade-off, not an oversight — a native dropdown handles many discrete options (34, here) far better than either a wall of duration buttons or a slider (imprecise on touch for 5-minute increments), but there's nowhere natural to put one in an instantaneous drop.
- There is still no way to change a block's duration *after* placement (true of every block type, not new to this feature) — Swap/Insights/Start/✕ remain the only per-block actions; picking a different duration means placing a new block via one of the two dialog-based paths above.

`placeWithOrder(dayIndex, insertAtIndex)` can't just trust pre-existing `sortOrder` values to find the block it just created (plenty of tap/drag-placed blocks all share `sortOrder: 0` from before any explicit reorder), so it creates the new block with a `sortOrder` deliberately higher than every existing block on that day, reloads, picks out whichever block on that day now has the *highest* `sortOrder` (unambiguous), splices it into the chosen slot among the rest, and renumbers the whole day sequentially — the exact same "rewrite every sortOrder 0..N-1" pattern `handleReorder()` already uses for drag-based reordering.

**Block-card actions are a footer button bar, not icons overlaid on the title row** (`.block-card-actions`/`.block-card-action-btn`). The overlaid-icon approach had two real problems: the title text had no truncation at all (a long title, especially a replaced-in exercise name, could overflow the card), and the icons sat directly in the same row a drag gesture would start from — spatially separating the buttons into their own row below the title/duration makes both the accidental-drag risk and the "which icon does what" ambiguity smaller by construction, on top of the SortableJS `filter` fix above. The title span now carries Tailwind's `truncate` inside a `min-w-0` flex container (the min-w-0 matters — a flex child won't shrink below its content's natural width without it, which is what let the overflow happen in the first place).

**Day/Week/Month views** (`viewMode`, `visibleDayIndices`, `monthGridWeeks`) all reuse the exact same board markup, save paths, and Sortable wiring — there's deliberately no separate "day view component" or parallel save logic to keep in sync:
- **Week** (default) renders `visibleDayIndices = [0,1,2,3,4,5,6]` — unchanged from before.
- **Day** renders `visibleDayIndices = [dayViewIndex]`, i.e. the identical `.day-column`/`.day-column-list` markup, just filtered to one column. `shiftDay()` wraps `dayViewIndex` across a week boundary by adjusting `weekStartDate` and reloading, same as `shiftWeek()` already did.
- **`initSortables()` maps Sortable's array position back to the real day-of-week through `visibleDayIndices[i]`, not `i` itself** — this is the one place Day view needed real wiring, not just a filtered `v-for`: Vue's `$refs.dayLists` array only has as many entries as were actually rendered (one, in Day view), so a naive `forEach((el, dayIndex) => ...)` would wire every drop in Day view to day 0 regardless of which day was actually showing. Both Day and Week view's drags/reorders save through the identical `handleDrop`/`handleReorder` → `Storage.addBlock`/`updateBlock` → `loadWeek()` path as a result — there's no separate "does Day view save correctly" concern, it's the same code path by construction.
- **Month** is read-only overview, not a drag target — dragging onto a tiny calendar cell isn't practical UX, so month cells only support click-to-navigate (`goToDay()`, switches to Day view for that date). Its data comes from `loadMonthView()`, which fetches every Monday-week overlapping the visible calendar grid via the same read-only `Storage.getWeekByDate()` the Daily Tracker uses (see below) — deliberately not `ensureWeek`, since just looking at a month shouldn't seed empty week rows for every week it happens to span.

**Day/Week board is a horizontally-scrolling row of minimum-width cards** (`.hscroll-card-row`, style.css), not a CSS-grid whose column count drops at each breakpoint. The grid approach let columns keep shrinking until their content truncated on narrower laptop screens — this replaces that with `flex: 1 0 448px` on `.day-column`: cards never shrink below 448px (matching Day view's own `max-w-md` single-card width, so Week and Day view read as the same card size), extra space distributes evenly across cards when there's room, and the row scrolls horizontally for whatever doesn't fit rather than cramming every day into view. Deliberately **not** applied to Month view — a calendar grid is supposed to shrink its cells to stay a single at-a-glance overview, and month cells (just a date + a count badge) have nothing dense enough to truncate anyway. This is meant as the standard pattern for any future multi-card row with the same problem, not a Week-Builder-only fix.

**`<main>`'s `max-w-4xl` prose-width cap is relaxed for the Week Builder specifically** (`index.html`, `:class="currentPage.isTrainingBuilder ? 'max-w-none' : 'max-w-4xl'"`). That cap is correct for long-form content pages (a readable line length), but it was also silently capping the day board's available width at ~896px regardless of actual screen width — on a genuinely wide monitor, the board was never able to use the extra space because of an unrelated prose-readability constraint. Scoped to just this one page rather than removed globally, since every prose/calculator page still benefits from the cap.
...
**Both `ensureTracker()` and `ensureWellness()` are `INSERT ... ON DUPLICATE KEY UPDATE id = id` followed by a SELECT, not a plain SELECT-then-INSERT** — the naive version let two overlapping requests for the same not-yet-existing row (e.g. a double-clicked "+ Add activity" button, or a Wellness save racing a screenshot upload) both see "nothing yet" and both attempt an INSERT, colliding on `daily_trackers`' `uniq_profile_date` key or `tracker_wellness`'s `uniq_wellness_tracker` key and crashing the loser with an unhandled `ER_DUP_ENTRY` (surfaced to users as a generic "Failed to add activity"). The atomic upsert makes concurrent callers converge on the same row instead of racing. The "+ Add activity" button also disables itself (`addingActivity`) while a request is in flight, so a double-click can't fire the race in the first place — belt-and-suspenders with the server-side fix, since the button guard alone wouldn't help two genuinely separate concurrent actions (e.g. Wellness save + screenshot upload).
...
Each placed block has four footer actions, not the two-icon overlay from earlier — see `.block-card-actions` in style.css for why the overlay was replaced (title overflow + drag-gesture conflict):
- **🔄 Swap** — unchanged from before, just renamed: `openBlockDetail()` opens the existing modal listing every `EXERCISE_TEMPLATES` entry in that block's category, "Use this →" replaces the block's title/`details.exerciseId`.
...
`sidebarCollapsed` is desktop-only (every class it touches is `lg:`-prefixed) and persisted to `localStorage` (`bp_sidebar_collapsed`) via a `watch`, same idea as the other `bp_*` client-side prefs. Separate from `sidebarOpen`, which is mobile's off-canvas drawer visibility — the two don't interact. Collapsing sets the `<aside>` to `lg:w-0 lg:opacity-0 lg:overflow-hidden` rather than `v-if`/`v-show`-ing it away: the main content area is its flex sibling (`class="flex min-h-screen"` on their shared parent), so shrinking the sidebar's width to 0 lets ordinary flexbox reflow the content to fill the freed space with no extra layout logic needed. Exists mainly for content-dense pages like the Week Builder's Week/Month views, which benefit from the extra width most.

**Mobile's drawer content is a group-tile grid + modal, not the desktop grouped list** (`.nav-groups-list` vs `.nav-group-grid`, toggled by the same plain-`@media`-query pattern as the Week Builder palette's category tiles/modal — see that section for the load-order rationale). The drawer (`sidebarOpen`, `w-72`) is narrow enough that the full 9-group/31-page list read as one long scroll before reaching a lower group; below 1024px it's replaced by a 2-column grid of group tiles (`activeNavGroupModal`), and tapping one opens a modal (reusing `.detail-modal-backdrop`/`.detail-modal`) listing just that group's pages. Selecting a page there calls the same `setPage(id)` as the desktop list — which already closes both `sidebarOpen` and (now) `activeNavGroupModal` in one call, since `setPage` sets `sidebarOpen = false` unconditionally and the modal-closing `activeNavGroupModal = null` is chained in the same click handler. Desktop (≥1024px) renders the original list, completely unchanged.
...
**Google Translate's widget injects stray chrome outside `#google_translate_element`, hidden via CSS as each one surfaces** (`style.css`, right after the widget's `<script>` block in `index.html`): `.goog-te-gadget-icon`/`.goog-logo-link`/`#goog-gt-tt`/`body > .skiptranslate` were already hidden from earlier issues, and `.VIpgJd-ZVi9od-aZ2wEe-wOHMyf` was added when the widget's floating circular icon — a fixed-position overlay injected straight into `<body>`, independent of the inline widget's own position in the sidebar footer — was found sitting directly over the mobile hamburger/sidebar-collapse toggle and blocking taps on it. That class is one of Google's own auto-generated build hashes, not a stable/semantic name, so it may need updating again after a future widget version change — if the sidebar toggle becomes unclickable again, re-inspect for a new stray floating element and add its class alongside the others.

...
**Palette categories are an accordion, collapsed by default** (`expandedCategories`, `toggleCategory()`) — `.palette-category-header` is now a `<button>`, not a plain label, showing an item count and toggling that category's items via `v-show` (same reasoning as the palette's own collapse above: keeps Sortable's DOM stable, no destroy/recreate). Before this, expanding the palette dumped all ~26 templates as one flat, single-column list on narrow screens (`.palette-item`'s `flex: 0 1 220px` only fits one per row under ~440px) — reaching a day column required scrolling past the *entire* palette first, which was the core of it being effectively unusable on mobile.
